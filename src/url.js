...
function(o, e) {
    ...
    c = e.EvpKDF = f.extend({
        ...
        compute: function(k, g) {
            for (var i = this.cfg,
            t = i.hasher.create(), l = a.create(), o = l.words, h = i.keySize, i = i.iterations; o.length < h;) {
                j && t.update(j);
...
window.CryptoJS.lib.Cipher ||
function(B) {
    var g = window.CryptoJS,
    j = g.lib,
    i = j.Base,
    D = j.WordArray,
    ...
    for (var C = window.CryptoJS,
    K = C.lib.BlockCipher,
    Q = C.algo,
    M = [], E = [], D = [], I = [], m = [], A = [], S = [], h = [], J = [], L = [], T = [], R = 0; 256 > R; R++) {
    ...
    Q = Q.AES = K.extend({
        _doReset: function() {
            ...
            r = b.words,
            q = b.sigBytes / 4,
            b = 4 * ((this._nRounds = q + 6) + 1), p = this._keySchedule = [], n = 0; n < b; n++) {
                if (n < q) {
                    p[n] = r[n]
                } else {
                    var l = p[n - 1];
        ...
        _doCryptBlock: function(ac, ab, aa, Z, Y, U, G, X) {
            ...
            W = ac[ab] ^ aa[0], V = ac[ab + 1] ^ aa[1], H = ac[ab + 2] ^ aa[2], z = ac[ab + 3] ^ aa[3], y = 4, w = 1; w < F; w++) {
    ...
    a.encryptquery = function(q, o) {
        try {
            var m = {};
            m.src = "http://pb.sogou.com/pv.gif?uigs_productid=weixin&type=encrypt&stype=2&wxtest=1&openid=" + encodeURIComponent(q) + "&hdq=" + o + "&wxsuv=" + b("SUV");
            if (typeof q == "undefined" && typeof o == "undefined") {
                return ""
            }
            if (q == null) {
                q = ""
            ...
            if (c == null || c == "" || c.length != 11 || d == null || d == "") {
                return "openid=" + encodeURIComponent(q) + "&hdq=" + o
            }
            var i = c;
            var n = g(o);
            i += n;
            ...
            var p = q + "hdq=" + o;
            var k = window.CryptoJS.AES.encrypt(p, i, {
                iv: h
            });
            var s = e(k.toString(), n);
            if (s == null || s == "") {
                return "openid=" + encodeURIComponent(q) + "&hdq=" + o
            }
            var j = encodeURIComponent(s.toString());
            var l = {};
            l.src = "http://pb.sogou.com/pv.gif?uigs_productid=weixin&type=encrypt&stype=4&wxtest=1&openid=" + encodeURIComponent(q) + "&hdq=" + o + "&wxsuv=" + b("SUV") + "&eqs=" + j + "&ekv=" + d;
            return "eqs=" + j + "&ekv=" + d
        } catch(r) {
            var m = {};
            m.src = "http://pb.sogou.com/pv.gif?uigs_productid=weixin&type=encrypt&stype=3&wxtest=1&ename=" + r.name + "&eline=" + r.line + "&openid=" + encodeURIComponent(q) + "&hdq=" + o + "&wxsuv=" + b("SUV");
            return "openid=" + encodeURIComponent(q) + "&hdq=" + o
        }
    };
